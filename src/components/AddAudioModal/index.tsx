...
const AddAudioModal = ({
    ...
    const [audioData, setAudioData] = useState({artist: "", title: "", img_url: "/img/NoMusicImg.png"})
    ...
    const isAudioUploadingError = useMusicStore(state => state.isAudioUploadingError)

    type TAddAudioStepArray = "LOAD_AUDIO_PAGE" | "EDIT_INFO_PAGE" | "UPLOADING_PAGE" | "SUCCESS_PAGE" | "ERROR_PAGE"
    const addAudioStepArray: TAddAudioStepArray[] = ["LOAD_AUDIO_PAGE", "EDIT_INFO_PAGE", "UPLOADING_PAGE", "SUCCESS_PAGE", "ERROR_PAGE"]


    const filePicker: React.MutableRefObject<any> = useRef(null);
    ...
    const inputOnChange = async (e: React.BaseSyntheticEvent) => {
        ...
        jsmediatags.read(file, {
            onSuccess: function (tag: any) {
                ...
                setAudioData({artist: fileTags.artist, title: fileTags.title, img_url: "/img/NoMusicImg.png"})
                setAddAudioStepArrayIndex(addAudioStepArrayIndex => addAudioStepArrayIndex + 1)
                // tag.tags.artist
                // tag.tags.title
            },
        ...
        >
            ...
            {addAudioStepArray[addAudioStepArrayIndex] === "EDIT_INFO_PAGE" && (
                <div className={styles.change_avatar_modal__box}>
                    ...
                    <div
                        className={styles.AddAudioModal__EDIT_INFO_PAGE__inner}
                    >
                        <div>
                            <p className={styles.AddAudioModal__title}>Никнейм артиста</p>
                            <input className={styles.AddAudioModal__input}
                                   value={audioData.artist}
                                   onChange={(e) => setAudioData({...audioData, artist: e.target.value})}/>
                        </div>
                        <div className="standart-gray-separator"/>
                        <div>
                            <p className={styles.AddAudioModal__title}>Название трека</p>
                            <input className={styles.AddAudioModal__input} value={audioData.title}
            ...
            {addAudioStepArray[addAudioStepArrayIndex] === "SUCCESS_PAGE" && (
                <div className={styles.change_avatar_modal__box}>
                    <div className={styles.change_avatar_modal__title}>
                        ...
                        {isAudioUploadingError ? "Аудиозапись не была добавлена по техническим причинам :(" : "Аудиозапись успешно добавлена!"}
                    </div>
                    <button
                        className={styles.change_avatar_modal__button}
                        onClick={onClose}
